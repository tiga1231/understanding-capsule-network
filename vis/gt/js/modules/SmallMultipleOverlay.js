function SmallMultipleOverlay(renderer) {
  ...
  this.init = function() {
    ...
    this.svg = d3.select('d-figure.'+renderer.gl.canvas.classList[0]+'>svg')
      .attr('width', width)
      .attr('height', height);
    ...
    if (this.controlOptionGroup === undefined) {
      this.controlOptionGroup = d3.select(
        'd-figure.'+renderer.gl.canvas.classList[0])
    ...
    if (this.clearBrushButton === undefined) {
      ...
      this.clearBrushButton = this.controlOptionGroup
        ...
        .attr('class', 'tooltip fas fa-sync-alt')
        .on('click', ()=>{
          this.clearAllBrushes();    
    ...
    if (this.fullScreenButton === undefined) {
      this.fullScreenButton = this.controlOptionGroup
        .insert('div', ':first-child')
        .append('i')
        .attr('id', 'smallmultiple-fullscreenButton')
        .attr('class', 'tooltip fas fa-expand-arrows-alt')
        .on('mouseover', function() {
          d3.select(this).style('opacity', 1);
        })
        .on('mouseout', function() {
    ...
    for (let j=0; j<renderer.ncol; j++) {
      let method = renderer.methods[j];
      for (let i=0; i<renderer.nrow; i++) {
        let epoch = renderer.epochs[i];
