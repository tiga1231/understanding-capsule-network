function SmallMultipleRenderer(gl, program, kwargs) {
  ...
  this.setKwargs = function(kwargs) {
    ...
    this.nrow = kwargs.epochs.length;
    this.ncol = kwargs.methods.length;
  };
  ...
  this.render = function() {
    ...
    for (let j=0; j<this.ncol; j++) {
      let dataTensor = dataObj[this.methods[j]];
      dataObj.points[this.methods[j]] = {};
      let dmax;
      let isDmaxConsistent;
      if (this.methods[j].includes('manual') 
          || this.methods[j].includes('random')
          || this.methods[j].includes('pca')) {
      ...
      for (let i=0; i<this.nrow; i++) {
        let points = dataTensor[this.epochs[i]];
        if (!isDmaxConsistent) {
        ...
        dataObj.points[this.methods[j]][this.epochs[i]] = points;

        gl.viewport( 
          j*sideLength, (this.nrow-(i+1))*sideLength, 
