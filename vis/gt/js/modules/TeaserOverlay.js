function TeaserOverlay(renderer) {
  ...
  this.slider = d3.select('.'+renderer.gl.canvas.id)
  ...
  this.playButton = d3.select('.'+renderer.gl.canvas.id)
  ...
  this.fullScreenButton = d3.select('.'+renderer.gl.canvas.id)
    .insert('i', ':first-child')
  ...
  this.grandtourButton = d3.select('.'+renderer.gl.canvas.id)
    .insert('i', ':first-child')
  ...
  this.svg = d3.select('.'+renderer.gl.canvas.id)
  ...
  this.controlOptionGroup = d3.select('.'+renderer.gl.canvas.id)
  ...
  this.updateArchorRadius = function(mode) {
    if (mode == 'point') {
      ...
      this.archorRadius = clamp(7, 20, Math.min(width, height)/30);
    }
  ...
  this.resize = function() {
    ...
    this.data_sx = d3.scaleLinear()
      .domain([-xmax, xmax])
      .range([0, width]);
    this.data_sy = d3.scaleLinear()
      .domain([-ymax, ymax])
      .range([height, 0]);
  ...
  this.drawAxes = function() {
    let svg = this.svg;
    let ndim = renderer.dataObj.ndim;
    ...
    let anchors = svg.selectAll('.anchor')
        .attr('cx', (d)=>this.data_sx(d[0]))
        .attr('cy', (d)=>this.data_sy(d[1]))
        .attr('r', this.archorRadius)
        .attr('fill', (_, i)=>'#333')
    ...
    svg.drag = d3.drag()
      ...
      .on('drag', (d, i)=>{
        let dx = this.data_sx.invert(d3.event.dx)-this.data_sx.invert(0);
        let dy = this.data_sy.invert(d3.event.dy)-this.data_sy.invert(0);
        let x = this.data_sx.invert(d3.event.x);
        let y = this.data_sy.invert(d3.event.y);
  ...
  this.redrawAxis = function() {
    ...
    if(renderer.gt !== undefined){
      ...
      svg.selectAll('.anchor')
        .attr('cx', (_, i) => this.data_sx(handlePos[i][0]))
        .attr('cy', (_, i) => this.data_sy(handlePos[i][1]));
    }
