function NeuralNetRenderer(svgid) {
  ...
  this.drawExamples = function() {
    ...
    let imageWidth = 28;
    let imageHeight = 28;
    let folder = 'data/examples/' + utils.getDataset() + '/input/';
    let data = [];
  ...
  this.getImageUrl = function(dataset, epochIndex, moduleName, imageIndex) {
    if (moduleName=='input') {
      return 'data/examples/'+dataset+'/'+moduleName+'/'+imageIndex+'.png';
    } else {
      return 'data/examples/'+dataset+'/epoch'+epochIndex+
  ...
  this.loadPred = function() {
    let promises = [
      d3.text('data/examples/'+utils.getDataset()+'/pred.csv'),
      d3.text('data/examples/'+utils.getDataset()+'/labels.csv')
    ];
